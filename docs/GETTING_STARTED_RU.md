...
## 0. Быстрый старт: двойной клик (Windows)

Больше не нужны два терминала и десятки команд.

1. Убедитесь, что установлен Rust (rustup). Если нет — один раз: скачайте `rustup-init.exe` с https://rustup.rs и поставьте с настройками по умолчанию.
2. Дважды кликните `start-solver.bat` в корне репозитория.
   - Первый запуск соберёт server (несколько минут), дальше — только старт.
   - После старта браузер сам откроется на `http://127.0.0.1:8080/`.
3. Работайте в UI. Это окно (`start-solver.bat`) держите открытым: закрыли окно — server остановился. `stop-solver.bat` — если нужно остановить принудительно.
4. После изменения кода проекта: `start-solver.bat rebuild`.

Данные jobs и checkpoints: `%USERPROFILE%\holdem-solver-data`.

### Как увидеть матрицу

Матрица — главный экран приложения, ничего отдельно открывать не нужно:

1. Нажмите `Настроить spot` (вверху справа) — откроются параметры стола.
2. Заполните игроков, button seat, blinds, stacks, hero seat и hero class (`AJo` — все 12 combos; `As Jd` — одна комбинация).
3. Добавьте историю действий (последний action должен оставить ход на hero).
4. Зада continuation ranges для каждого seat, при необходимости — размеры ставок в дереве.
5. `Проверить spot` — убедиться, что validate прошёл (иначе solve запустится с ошибкой).
...
Рекомендуемый путь — `start-solver.bat` (раздел 0). Ручной запуск эквивалентен ему:
